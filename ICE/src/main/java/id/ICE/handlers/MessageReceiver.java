...
/**
 * This handler acts as a message receiver and is notified each time server reads some data from the
 * client.
 *
 * <p>All read requests are done asynchronously. It means that this receiver may be called with
 * different amounts of data read each time.
 *
 * <p>Receiver relies on message scanner to extract a message from a stream of bytes and then it
 * passes them back to the looper.
 *
 * <p>Each time receiver reads new portion of data it appends it to internal buffer and then it uses
 * scanner to tell where message ends in this buffer and another begins.
 *
...
public class MessageReceiver implements CompletionHandler<Integer, AsynchronousSocketChannel> {
    ...
    /** Called once read operation succeeds with some data read or when read is failed. */
    @Override
    public void completed(Integer result, AsynchronousSocketChannel channel) {
        ...
        if (channel.isOpen()) channel.read(buf, channel, this);
    }
    ...
    public CompletableFuture<ByteBuffer> receive() {
        ...
        if (buf.position() == 0 || !processMessage()) channel.read(buf, channel, this);
        return future;
    }
    ...
    private boolean processMessage() {
        ...
        buf = bbUtils.copyToHead(buf, pos, buf.position());
